...
//  require_once($_SERVER["DOCUMENT_ROOT"] . "/phpreg/lib/debug_inc.php");
  ...
  $curraction = "/phpreg/chpass.php";
  if (isset($_SERVER["HTTP_X_FORWARDED_FOR"])) {
    $curraction = "/phpreg/chpass.php";
//    echo "Password changes are temporarily disabled."; die();
  }

  // Local variables
  $regform = new HTML_QuickForm2("regform", "post", "", "", NULL, true);
  $qform =& $regform;
  $elem = array();
  ...
  function build_validation(&$form, $client_validate=HTML_QuickForm2_Rule::CLIENT_SERVER) {
    ...
    $elem["login"]->addRule("callback", "Login name doesn't exist.", "acct_exist");
  ...
  $elems =& build_regform2($regform, $reglabel);
  ...
  if ($regform->isSubmitted()) {
    if ($regform->validate()) {
      ...
      $GLOBALS["tpl"]->assign("blanket_err", "There was an error processing your password change, please review the items marked in red below and correct before submitting again.");
    }
  }
